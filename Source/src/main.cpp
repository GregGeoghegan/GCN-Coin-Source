...
uint256 hashGenesisBlock("0xae3d22ef008df87695dd2681c24294a9456a33694c2dcfb098f743def5c0bd45");
...
{
    // GCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if (nHeight == 1)
		return 40000000000 * COIN;
		
	// Subsidy halving
    nSubsidy >>= (nHeight / 533333);
...
{
    // GCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		// Genesis block
        const char* pszTimestamp = "GCoin - Mar 22 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a3314913115373a129cff79907ba4a03d351873d9e3acb6c973d3c9e3c88301a3d3c94a600deac244b91704e7b4ffb30ad91c8e56e695a1ddf318492988afe0a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1395491911;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xf70d026a0434a0b75639ed06af05f2bccae9544bf6ffecf8b1431c13f356e1c4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd8, 0xc7, 0xef, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GCoinMiner(CWallet *pwallet)
{
    printf("GCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GCoinMiner, pwallet));
}
